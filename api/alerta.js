...
function getVecinos() {
  try {
    return JSON.parse(process.env.VECINOS || '[]');
  } catch {
    return [];
  }
}
...
module.exports = async (req, res) => {
  ...
  if (!parcela) {
    return res.status(400).json({ error: 'Falta número de parcela' });
  }

  const vecinos = getVecinos();
  if (vecinos.length === 0) {
    return res.status(500).json({ error: 'No hay vecinos registrados' });
  }
  ...
  const llamadas = vecinos.map(async (vecino) => {
    try {
  ...
  console.log(`[${timestamp}] ALERTA Parcela ${parcela} - ${resultados.length} llamadas iniciadas`);

  return res.status(200).json({
    ok: true,
    parcela,
    timestamp,
    llamadas: resultados.length,
    detalle: resultados
  });
};
